...
impl<F: FileSystem> KeyService<F> {
    ...
    pub fn save_signing_key(
        ...
        info!("SAVE: salt raw: {}", user.user_salt);
        info!("SAVE: salt from get_salt: {:?}", user.get_salt()?);
        info!("SAVE: encryption key: {:?}", encryption_key);
    ...
    pub fn load_signing_key(
        ...
        let mut encryption_key = derive_key_from_password(raw_password, user)?;

        info!("LOAD: salt raw: {}", user.user_salt);
        info!("LOAD: salt from get_salt: {:?}", user.get_salt()?);
        info!("LOAD: encryption key: {:?}", encryption_key);

        let encrypted = self.fs.read_file(&sk_path_str)?;
        info!("LOAD: encrypted value {:?}", &encrypted);

        let mut decrypted = decrypt_data(&encrypted, &encryption_key)?;
        info!("LOAD: decrypted data length: {}", decrypted.len());
        ...
        if decrypted.len() == 32 {
            ...
            log::error!(
                "Incorret decrypted key lenght: expect 32, got {}",
                decrypted.len()
            );
    ...
    pub fn load_verifying_key(&self, user: &User) -> Result<VerifyingKey, AppError> {
        ...
        info!("Verifying key file exists");
        ...
        info!("Base64 encoded key read: {} bytes", encoded.len());
        ...
        info!("Decodes key bytes: {} bytes", bytes.len());

        if bytes.len() != 32 {
            error!("Invalid key length: expected 32, got {}", bytes.len());
